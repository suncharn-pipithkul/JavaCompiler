...
public class Parser {
    ...
    /**
     * Parses a interface body and returns an ArrayList.
     *
     * <pre>
     *     interfaceBody  ::=  LCURLY { modifiers  interfaceMemberDecl } RCURLY
     * </pre>
     *
     * @return an ArrayList of interface body.
     */
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        ...
        // Implicit modifiers for interface
        if (mods == null)
            mods = new ArrayList<>();
        mods.add("abstract");
        mods.add("public");
        ...
        if (have(VOID)) {
            ...
            if (have(THROWS)) {
                ...
                TypeName typeName = qualifiedIdentifier();
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        // IDENTIFIER formalParameters [ THROWS qualifiedIdentifier
        // { COMMA qualifiedIdentifier } ] block
        if (seeIdentLParen()) {
            // A constructor.
            mustBe(IDENTIFIER);
            String name = scanner.previousToken().image();
    ...
    private JStatement statement() {
        ...
        // FOR  LPAREN [ forInit ] SEMI [ expression ] SEMI [ forUpdate ] RPAREN  statement
            ...
            if (!see(SEMI)) { // if the next char is not ; , then there is init
                init = forInit();
            }
            mustBe(SEMI);
            JExpression condition = null;
            if (!see(SEMI)) { // if the next char is not ; , then there is expression
                condition = expression();
            }
            mustBe(SEMI);
            ArrayList<JStatement> update = null;
            if (!see(RPAREN)) { // if the next char is not ) , then there is expression
                update = forUpdate();
            }
            mustBe(RPAREN);
            JStatement body = statement();
        ...
        //  TRY  block { CATCH  LPAREN  formalParameter  RPAREN  block } [ FINALLY  block ]
            ...
            do { // { CATCH LPAREN formalPara RPAREN block }
                mustBe(CATCH);
                mustBe(LPAREN);
    ...
    /**
     * forInit statement and returns a list of expressions.
     *
     * <pre>
    ...
    private ArrayList<JStatement> forInit() {
        ArrayList<JStatement> statements = new ArrayList<JStatement>();
        // If it's a list of variables declarations
        ...
        } else { // else it's a list of statements
            do {
    ...
    /**
     * forUpdate statement and returns a list of expressions.
     *
     * <pre>
    ...
    /**
     * SwitchStatementGroup returns a group of switch case and block
     *
     * <pre>
    ...
    private SwitchStatementGroup switchBlockStatementGroup() {
        ...
        } while(see(CASE) || see(DEFLT)); // 1 or more occurrences

        while(!see(CASE) && !see(DEFLT) && !see(RCURLY)) { // 0 or more occurrences
    ...
    /**
     * switchLabel returns an JExpression of each switch label.
     *
     * <pre>
     ...
     *
     * <pre>
     *   conditionalExpression ::= conditionalExpression [ QUESTION expression COLON conditionalExpression ]
     * </pre>
     *
    ...
    /**
     * Parses a bit  or expression and returns an AST for it.
     *
     * <pre>
    ...
    /**
     * Parses a bit exclusive or expression and returns an AST for it.
     *
     * <pre>
    ...
    /**
     * Parses a bit and expression and returns an AST for it.
     *
     * <pre>
     ...
     *
     * <pre>
     *   shiftExpression ::= additiveExpression { ( ALSHIFT | ARSHIFT | LLSHIFT ) additiveExpression }
     * </pre>
     *
     * @return an AST for a relational expression.
     */
