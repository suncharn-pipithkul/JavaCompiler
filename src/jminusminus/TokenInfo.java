...
enum TokenKind {
    ...
    CATCH("catch"), CHAR("char"), CLASS("class"), CONTINUE("continue"), DEFLT("default"),
    DO("do"), DOUBLE("double"),
    ELSE("else"), EXTENDS("extends"), FINALLY("finally"),
    FOR("for"), IF("if"), IMPLEMENTS("implements"), IMPORT("import"), INSTANCEOF("instanceof"),
    INT("int"), INTERFACE("interface"), LONG("long"),
    ...
    // Operators.
    ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), INC("++"), LAND("&&"), LE("<="),
    BITNOT("~"), LNOT("!"), MINUS("-"), PLUS("+"), PLUS_ASSIGN("+="), STAR("*"), DIV("/"), REM("%"), BITOR("|"),
    BITXOR("^"), BITAND("&"), ALSHIFT("<<"), ARSHIFT(">>"), LRSHIFT(">>>"), NOT_EQUAL("!="),
    DIV_ASSIGN("/="), MINUS_ASSIGN("-="), STAR_ASSIGN("*="), REM_ASSIGN("%="),
    ARSHIFT_ASSIGN(">>="), LRSHIFT_ASSIGN(">>>="), GE(">="), ALSHIFT_ASSIGN("<<="), LT("<"),
    XOR_ASSIGN("^="), OR_ASSIGN("|="), LOR("||"), AND_ASSIGN("&="), QUESTION("?"), COLON(":"),
